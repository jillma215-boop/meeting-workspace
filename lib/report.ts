import { WeekOption, getDefaultWeekOption, getIsoWeekInfo } from './week';
...
export type Report = {
  id: string; type: ReportType; weekYear: number; weekNumber: number; startDate: string; endDate: string; status: ReportStatus; updatedAt: string;
  author: string; version: number; shareId?: string; sharedAt?: string | null; isShared: boolean;
...
const COMPAT_STORAGE_KEYS = ['rakumon-reports', 'rakumon-weekly-reports'];
export const kpiGroups = [
  { title: '売上・課金', labels: ['一般売上', '法人売上', '新規課金者数', '継続課金者数'] },
  { title: '利用状況', labels: ['新規登録ユーザー', '利用生徒数', '質問数'] },
  { title: '回答・品質', labels: ['回答数', '回答完了率', '問い合わせ件数'] },
] as const;
export const kpiLabels = kpiGroups.flatMap(group => [...group.labels]);
...
export const createBlankReport = (type: ReportType = 'weekly', targetWeek?: WeekOption): Report => {
  const now = new Date(); const week = targetWeek ?? getDefaultWeekOption(now);
  return { id: uid(), type, weekYear: week.weekYear, weekNumber: week.weekNumber, startDate: week.startDate, endDate: week.endDate, status: 'Draft', updatedAt: now.toISOString(), author: 'Jill', version: 1, shareId: undefined, sharedAt: null, isShared: false, overview: blankOverview(), kpis: kpiLabels.map(label=>({label,value:'',comparison:'',comment:''})), studentInquiry: emptyInquiry(), teacherInquiry: emptyInquiry(), tob: [{id: uid(), project:'', status:'', weeklyAction:'', confirmation:'', nextAction:''}], operations: blankOps(), nextActions: [{id: uid(), item:'', owner:'', due:'', status:'', note:''}] };
};
export const normalizeReport = (raw: Partial<Report>): Report => {
  ...
  const iso = getIsoWeekInfo(raw.startDate ?? base.startDate);
  return { ...base, ...raw, weekYear: typeof raw.weekYear === 'number' ? raw.weekYear : iso.weekYear, weekNumber: typeof raw.weekNumber === 'number' ? raw.weekNumber : iso.weekNumber, type: raw.type === 'monthly' || raw.type === 'project' ? raw.type : 'weekly', status: raw.status === 'Completed' || (raw.status as string | undefined) === 'completed' ? 'Completed' : 'Draft', author: raw.author ?? base.author, version: typeof raw.version === 'number' ? raw.version : base.version, shareId: raw.shareId, sharedAt: raw.sharedAt ?? null, isShared: raw.isShared === true, overview, kpis, studentInquiry: { ...emptyInquiry(), ...(raw.studentInquiry ?? {}) }, teacherInquiry: { ...emptyInquiry(), ...(raw.teacherInquiry ?? {}) }, tob: Array.isArray(raw.tob) ? raw.tob : base.tob, operations: { ...blankOps(), ...(raw.operations ?? {}) }, nextActions: Array.isArray(raw.nextActions) ? raw.nextActions : base.nextActions };
};
export const sampleReport: Report = normalizeReport({ id: 'sample-rakumon-weekly', type: 'weekly', startDate: '2026-06-22', endDate: '2026-06-28', status: 'Draft', updatedAt: new Date().toISOString(), overview: { summary: '問い合わせ対応は安定。ToB案件は確認事項を来週前半に解消し、導入準備を進める。', topics: [{title:'回答品質の安定',description:'先生回答の一次確認フローにより回答完了率が改善傾向。'},{title:'法人向け資料更新',description:'導入検討企業向けのFAQと運用イメージを更新。'},{title:'夏期講習前の準備',description:'質問増加に備えた運営シフト案を作成。'}], issues: '未対応問い合わせの滞留が一部発生。仕様確認が必要な案件は開発側との同期を早める。', nextFocus: '未対応チケットの翌営業日解消、ToB確認事項の整理、夏期繁忙期の体制確定。' }, kpis: kpiLabels.map((label,i)=>({label,value:['¥920,000','¥320,000','84名','612名','324名','298名','1,486件','1,431件','96.3%','128件'][i],comparison:['+8%','+14%','+6%','+3%','+6%','+5%','+12%','+10%','-0.8pt','-4%'][i],comment:'週次定例で確認'})), studentInquiry: { total:'128件', top3:['解説内容の確認','決済・プラン変更','学習進捗の相談'], examples:'「途中式をもう少し詳しく知りたい」など。', resolved:'116件', pending:'12件', insight:'解説粒度への期待が高く、テンプレート改善余地あり。' }, teacherInquiry: { total:'42件', top3:['回答方針確認','稼働シフト','報酬確認'], examples:'難問時のエスカレーション基準に関する確認。', resolved:'39件', pending:'3件', insight:'新任先生向けオンボーディング資料の補強が必要。' }, tob: [{id:'tob-1',project:'A塾トライアル',status:'提案中',weeklyAction:'運用FAQを送付',confirmation:'利用開始日',nextAction:'7/2条件確認'},{id:'tob-2',project:'B社研修',status:'要件整理',weeklyAction:'ヒアリング実施',confirmation:'対象学年',nextAction:'見積更新'}], operations: { done:'問い合わせ分類の見直し、FAQ更新、先生向け周知を実施。', issues:'仕様確認待ちチケットのリードタイム。', improvements:'問い合わせタグを統一し、集計しやすくした。', development:'管理画面の検索条件追加を相談中。', risks:'夏期の質問増加に対する回答体制不足。' }, nextActions: [{id:'act-1',item:'未対応問い合わせ棚卸し',owner:'運営',due:'2026-07-01',status:'進行中',note:'優先度高から対応'},{id:'act-2',item:'ToB確認事項整理',owner:'Biz',due:'2026-07-03',status:'未着手',note:'A塾・B社'}] });
export function loadReports(): Report[]{
  ...
  const loadKey = (key: string) => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) parsed.map(normalizeReport).forEach((report) => byId.set(report.id, report));
    } catch {}
  };
  loadKey(STORAGE_KEY);
  COMPAT_STORAGE_KEYS.forEach(loadKey);
  if(byId.size === 0){localStorage.setItem(STORAGE_KEY, JSON.stringify([sampleReport])); return [sampleReport];}
