...
export const createBlankReport = (type: ReportType = 'weekly'): Report => {
  ...
  return { id: uid(), type, startDate: start.toISOString().slice(0,10), endDate: end.toISOString().slice(0,10), status: 'Draft', updatedAt: now.toISOString(), overview: blankOverview(), kpis: kpiLabels.map(label=>({label,value:'',comparison:'',comment:''})), studentInquiry: emptyInquiry(), teacherInquiry: emptyInquiry(), tob: [{id: uid(), project:'', status:'', weeklyAction:'', confirmation:'', nextAction:''}], operations: blankOps(), nextActions: [{id: uid(), item:'', owner:'', due:'', status:'', note:''}] };
};
